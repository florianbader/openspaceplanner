...
public static class DeleteTimeSlot
{
    ...
    public class Controller(IMediator mediator) : TimeSlotControllerBase
    {
        ...
        [HttpDelete("{id}")]
        public Task DeleteAsync(SessionId sessionId, TimeSlotId id, CancellationToken cancellationToken) =>
    ...
    public class Handler(DbContext databaseContext, ILogger<Handler> logger) : IRequestHandler<Command>
    {
        public async Task Handle(Command request, CancellationToken cancellationToken)
        {
            await databaseContext
                .Set<TimeSlot>()
                .TagWithCallSite(nameof(DeleteTimeSlot))
    ...
    public record Command(SessionId SessionId, TimeSlotId Id) : IRequest;
}
